...
REPORT_PATH = PROJECT_ROOT / "data" / "emlak_katilim_initial_integration_report.json"
...
def validate_source_files(expected: int | None) -> dict[str, int]:
    ...
    if discovery_errors:
        raise RuntimeError(
            f"Keşif hata kaydı var: {len(discovery_errors)}"
        )
    if fetch_errors:
        raise RuntimeError(
            f"Fetch hata kaydı var: {len(fetch_errors)}"
        )
...
def repair_snapshot_statuses() -> dict[str, int]:
    """
    Son discovery/fetch sonrası snapshot ve index status alanlarını,
    güncel tarih parser'ı ve listing_status kanıtıyla yeniden hesaplar.
    """
    ...
    for row in rows:
        ...
        if not snapshot_value:
            raise RuntimeError(
                "Emlak indeks kaydında snapshot_file boş: "
        ...
        result = evaluate_campaign_status(
            ...
            listing_evidence=str(
                snapshot.get("listing_status_evidence")
...
def make_backup() -> Path:
    ...
    backup = (
        ...
        / "data"
        / "backups"
        / f"campaigns_before_emlak_initial_{stamp}.db"
    )
...
def db_quality() -> dict:
    ...
    try:
        ...
        if not rows:
            raise RuntimeError(
                "DB'de Türkiye Emlak Katılım kaydı oluşmadı."
            )
        ...
        review = [
            ...
            for row in rows
            if (
                str(row["record_kind"] or "") != "campaign"
                or str(row["campaign_category"] or "") in {
                    "",
                    "unclassified",
                }
                or str(row["current_status"] or "") == "unknown"
            )
        ]
        ...
        return {
            "db_records": len(rows),
            "active_records": sum(
                str(row["current_status"] or "") == "active"
                for row in rows
            ),
            "expired_records": sum(
                str(row["current_status"] or "") == "expired"
                for row in rows
            ),
            "upcoming_records": sum(
                str(row["current_status"] or "") == "upcoming"
                for row in rows
            ),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Türkiye Emlak Katılım'ın keşfedilip fetch edilmiş "
            "kampanyalarını DB/dashboard pipeline'ına ilk kez "
            "güvenli biçimde alır. Canlı scanner'ı henüz açmaz."
        )
    )
    parser.add_argument(
        "--expected",
        type=int,
        default=65,
        help="İlk entegrasyonda beklenen benzersiz keşif sayısı.",
    )
    args = parser.parse_args()

    print("=" * 78)
    print("TÜRKİYE EMLAK KATILIM — İLK DASHBOARD ENTEGRASYONU")
    print("=" * 78)
    print("Mevcut keşif + fetch snapshot'ları kullanılır.")
    print("Kaldırma/pasife alma yapılmaz.")
    print("scanner_ready değiştirilmez.")
    ...
    try:
        ...
        run(
            [
                ...
                BANK,
                "--report",
                "data/emlak_katilim_comparison_extraction_report.json",
            ],
        ...
        if quality["duplicate_url_groups"]:
            raise RuntimeError(
                "DB'de Emlak Katılım mükerrer URL grubu bulundu."
            )

        if quality["review_count"]:
            raise RuntimeError(
                "DB kalite kontrolünde manuel inceleme gereken "
                f"{quality['review_count']} kayıt var."
            )

        if quality["statuses"].get("unknown", 0):
            raise RuntimeError(
                "DB'de unknown status kaldı."
            )
        ...
        report = {
            ...
            "next_step": (
                "DB kalite çıktısı doğrulandıktan sonra "
                "scanner_ready=true yapılıp canlı updater testi."
            ),
        }
        save_json(REPORT_PATH, report)

        print("\n" + "=" * 78)
        print("TÜRKİYE EMLAK KATILIM İLK ENTEGRASYON TAMAMLANDI")
        print("=" * 78)
