...
def test_official_savings_sources_are_direct_campaign_pages():
    bank = turkiye_finans_config()

    urls = {
        item["url"].casefold()
        for item in bank["campaign_sources"]
        if item["source_group"] == "Türkiye Finans Birikim / Fon Kampanyaları"
    }
    ...
    assert not any("birikim-fon-kampanyalari.aspx" in url for url in urls)
