...
def test_other_bank_fingerprint_supports_table_without_id(
    tmp_path,
):
    db_path = tmp_path / "campaigns.db"
    connection = sqlite3.connect(db_path)
    ...
    fingerprint = other_banks_fingerprint(
        db_path,
        "Türkiye Finans",
    )

    assert fingerprint["live_campaigns"]
    assert fingerprint["live_campaign_finance_details"]
    assert fingerprint["live_campaign_benefits"]
    assert fingerprint["live_campaign_audiences"]
