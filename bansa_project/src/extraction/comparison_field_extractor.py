...
def extract_maturities(
    ...
    # Ana kampanya metnindeki "6 taksite kadar" ifadesi,
    # örnek ödeme planındaki "3 ay vadeli" bilgisinden daha
    # güçlü bir kampanya vadesi göstergesidir.
    if installment_values:
        max_installment = max(installment_values)
        if not values or max_installment > max(values):
...
def extract_finance_fields(
    ...
    advantage, evidence = extract_campaign_advantage(title, text)
...
def extract_benefits(
    ...
    full_text = normalize_text(
        f"{title} {clean_content_text(title, text)}"
    )
    ...
    point_matches = list(
        re.finditer(
            (
                rf"{money}\s*(?:TL|₺)?\s*"
                r"(?:Altın\s+Puan|Worldpuan|WorldPuan|puan)"
            ),
            full_text,
            flags=re.IGNORECASE,
        )
    ...
    if point_values or qualitative_points_evidence:
        results.append(
            BenefitExtraction(
                ...
                None,
                (
                    "Altın Puan, Worldpuan veya alışveriş puanı"
                    if point_values
...
def extract_audiences(
    ...
    *,
    ...
    for terms, audience_type, label in AUDIENCE_RULES:
        if any(search_key(term) in folded for term in terms):
            if audience_type not in seen:
