...
except ModuleNotFoundError:  # Portable/demo runtime may not use PostgreSQL.
    psycopg = None

    def Jsonb(value):
        # Keep non-Postgres unit/portable paths importable. Real PostgreSQL
        # writes still require psycopg and are guarded in connect_postgres().
        return value
...
def connect_postgres():
    ...
    if psycopg is None:
        raise RuntimeError(
            "PostgreSQL bağlantısı için 'psycopg' paketi kurulu olmalı."
        )
