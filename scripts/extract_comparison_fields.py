...
from src.extraction.installment_terms_extractor import (
    extract_installment_terms,
)
...
KUVEYT_HEPSIBURADA_BANK = "Kuveyt T\u00fcrk"
KUVEYT_HEPSIBURADA_URL = 'https://kuveytturk.com.tr/kampanyalar/kendim-icin/musteri-ol-kampanyalari/hepsiburadada-yeni-musteriye-ozel-vade-farksiz-50000-tl-firsati'


def apply_kuveyt_hepsiburada_rate_guardrail(
    finance,
    *,
    bank_name: str,
    source_url: str,
):
    """A??k oran bulunmayan Hepsiburada kayd?ndaki
    yanl?? y?zde s?f?r ??kar?m?n? temizler."""
    normalized_url = str(source_url or "").rstrip("/")

    if (
        bank_name == KUVEYT_HEPSIBURADA_BANK
        and normalized_url
        == KUVEYT_HEPSIBURADA_URL.rstrip("/")
    ):
        return replace(
            finance,
            profit_share_rate_min=None,
            profit_share_rate_max=None,
            profit_share_rate_text=None,
            campaign_advantage=(
                "Hepsiburada alışverişlerinde "
                "50.000 TL'ye kadar vade farksız 9 taksit."
            ),
            evidence_text=(
                "Kampanya metninde 50.000 TL'ye kadar "
                "9 taksit avantaj? belirtilmektedir; "
                "say?sal k?r pay? oran? a??k?a "
                "belirtilmemi?tir."
            ),
        )
...
def ensure_schema(connection: sqlite3.Connection) -> None:
    ...
    connection.executescript(
        ...
        CREATE TABLE IF NOT EXISTS live_campaign_installment_terms (
            campaign_id INTEGER PRIMARY KEY,
    ...
    connection.execute(
        ...
        SELECT
            ...
            installment.minimum_transaction_amount,
            installment.maximum_transaction_amount,
            installment.installment_count
                AS card_installment_count,
            installment.installment_cost_rate,
            installment.installment_cost_text,
            MAX(
                CASE
        ...
        LEFT JOIN live_campaign_installment_terms AS installment
            ON installment.campaign_id = campaign.id
...
def upsert_installment_terms(
    ...
    has_values = any(
        value is not None
        for value in (
            extraction.minimum_transaction_amount,
            extraction.maximum_transaction_amount,
            extraction.installment_count,
            extraction.installment_cost_rate,
            extraction.installment_cost_text,
        )
    )

    if not has_values:
        connection.execute(
            "DELETE FROM live_campaign_installment_terms "
            "WHERE campaign_id = ?",
            (campaign_id,),
        )
        return False
    ...
    connection.execute(
        """
        INSERT INTO live_campaign_installment_terms (
            campaign_id,
        ...
        ON CONFLICT(campaign_id) DO UPDATE SET
            minimum_transaction_amount =
                excluded.minimum_transaction_amount,
            maximum_transaction_amount =
                excluded.maximum_transaction_amount,
            installment_count =
                excluded.installment_count,
            installment_cost_rate =
                excluded.installment_cost_rate,
            installment_cost_text =
                excluded.installment_cost_text,
            evidence_text =
...
def main() -> int:
    ...
    try:
        ...
        with connection:
            for row in rows:
                ...
                if category == "finance_campaign":
                    extracted_finance = extract_finance_fields(
                        title,
                        clean_text,
                    )
                    ...
                    finance = apply_kuveyt_hepsiburada_rate_guardrail(
                        finance,
                        bank_name=row["bank_name"],
                        source_url=row["source_url"],
                    )
                ...
                installment_terms = extract_installment_terms(
                    title,
                    clean_text,
                )
                if upsert_installment_terms(
                    connection,
                    campaign_id,
                    installment_terms,
                    timestamp,
                ):
                    installment_terms_count += 1
        ...
        report = {
            ...
            "installment_terms_rows": installment_terms_count,
        ...
        print(
            "Taksit/işlem koşulu kaydı:",
            installment_terms_count,
        )
        print(
