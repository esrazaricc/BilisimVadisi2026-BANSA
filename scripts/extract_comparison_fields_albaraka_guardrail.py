...
ALBARAKA_TAKSITLIO_URL = (
    "https://albaraka.com.tr/tr/kampanyalar/detay/"
    "taksitliocom-alisveris-finansmani"
)
ALBARAKA_TAKSITLIO_FINANCE_TYPE = "Alışveriş Finansmanı"


def apply_finance_type_guardrail(
    extraction,
    *,
    bank_name: str,
    source_url: str,
    ...
    """
    Doğrulanmış banka/URL bazlı finansman türü düzeltmelerini uygular.

    Genel anahtar kelime eşleşmesi kullanılmaz; böylece diğer bankalar
    ve benzer başlıklı kampanyalar etkilenmez.
    """
    normalized_url = str(source_url or "").strip().rstrip("/").casefold()
    target_url = ALBARAKA_TAKSITLIO_URL.rstrip("/").casefold()

    if (
        bank_name == "Albaraka Türk"
        and normalized_url == target_url
    ):
        changed = (
            extraction.finance_type
            != ALBARAKA_TAKSITLIO_FINANCE_TYPE
        )
        return (
            replace(
                extraction,
                finance_type=ALBARAKA_TAKSITLIO_FINANCE_TYPE,
            ),
            changed,
        )
...
def main() -> int:
    ...
    try:
        ...
        finance_type_guardrail_count = 0
        benefit_count = 0
        audience_count = 0
        ...
        with connection:
            for row in rows:
                ...
                if category == "finance_campaign":
                    ...
                    finance, type_guardrail_applied = (
                        apply_finance_type_guardrail(
                            finance,
                            bank_name=row["bank_name"],
                            source_url=row["source_url"],
                        )
                    )
                    if type_guardrail_applied:
                        finance_type_guardrail_count += 1
        ...
        report = {
            ...
            "finance_type_guardrail_rows": (
                finance_type_guardrail_count
            ),
        ...
        print(
            "Finansman türü guardrail düzeltmesi:",
            finance_type_guardrail_count,
        )
