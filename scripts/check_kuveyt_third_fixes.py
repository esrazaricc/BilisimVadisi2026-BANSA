...
def main() -> int:
    ...
    current_finance_count = connection.execute(
        """
        SELECT COUNT(*)
        FROM live_campaigns
        WHERE bank_name = ?
    ...
    print("Güncel finansman kampanyası:", current_finance_count)
    print("Finansman detay kaydı:", len(finance_rows))

    if len(finance_rows) != current_finance_count:
        errors.append(
            "Güncel finance_campaign sayısı ile finansman detay "
            f"kaydı eşleşmiyor: kampanya={current_finance_count}, "
            f"detay={len(finance_rows)}."
        )
    ...
    if not hepsi or (
        ...
        or hepsi["profit_share_rate_text"] is not None
