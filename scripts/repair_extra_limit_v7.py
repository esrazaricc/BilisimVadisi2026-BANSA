...
def main() -> int:
    ...
    # eXtra Limit resmî maliyet tablosu altındaki teknoloji ürünü
    # taksit sınırlarını eksiksiz tut. 100 TL burada minimum
    # finansman tutarı değildir; yalnız minimum taksitlendirme tutarıdır.
    category_rules = [
        rule for rule in rules["category_rules"]
        if str(rule.get("category_key") or "") != "bilgisayar"
    ]
    if re.search(r"bilgisayar\s+al[ıi]mlar[ıi]\s+12", text, flags=re.IGNORECASE):
        category_rules.append(
            {
                "category_key": "bilgisayar",
                "category_label": "Bilgisayar",
                "min_amount": None,
                "max_amount": None,
                "min_inclusive": False,
                "max_inclusive": True,
                "max_installments": 12,
                "max_maturity_months": None,
                "condition_text": "Maksimum 12 taksit",
                "source_text": "Bilgisayar alımları 12 taksit ile sınırlandırılmıştır.",
            }
        )
    rules["category_rules"] = category_rules
    target["finance_rules_json"] = json.dumps(
        rules, ensure_ascii=False, sort_keys=True
    )

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup = JSON_PATH.with_name(f"turkiye_finans_before_extra_limit_v7_{stamp}.json")
