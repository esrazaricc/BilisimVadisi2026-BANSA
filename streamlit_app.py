...
with st.sidebar:
    ...
    include_noncurrent = st.checkbox(
        "Süresi dolmuş / yaklaşan kayıtları da göster",
        value=False,
    )
    st.divider()
...
campaigns["title"] = campaigns.apply(
...
# Varsayılan dashboard yalnızca gerçekten aktif kampanyaları gösterir.
# "is_current=1" kaydın hâlâ kaynakta bulunduğunu belirtir; süresi dolmuş
# bir kampanya da geçmiş kaydı korumak için is_current=1 kalabilir.
if not include_noncurrent:
    visible_campaigns = visible_campaigns[
        (visible_campaigns["is_current"] == 1)
        & (visible_campaigns["current_status"] == "active")
    ].copy()
...
metric_columns[0].metric(
    ...
    int(
        (
            ...
            & (campaigns["current_status"] == "active")
        ).sum()
    ),
)
    ...
    if not filtered.empty:
        ...
        with st.expander(
            ...
            st.markdown(
                ...
                f"{record['classification_reason'] or 'Belirtilmedi'}"
            )
            st.text_area(
...
with tabs[2]:
    ...
    if not include_noncurrent:
        finance = finance[
            (finance["is_current"] == 1)
            & (finance["current_status"] == "active")
        ].copy()
...
with tabs[3]:
    ...
    with benefit_tab:
        ...
        selected_benefit_types = st.multiselect(
            ...
            format_func=label_benefit_type,
        )
        ...
        benefit_display["Fayda Türü"] = benefit_display[
            "Fayda Türü"
        ].map(label_benefit_type)
        dataframe_with_link(benefit_display)

    with audience_tab:
        audiences = load_audiences(selected_banks)
        if not audiences.empty:
            audiences["title"] = audiences.apply(
                lambda row: clean_campaign_title(
                    row["title"],
                    row["bank_name"],
                ),
...
with tabs[5]:
    ...
    if changes.empty:
        ...
        change_display["Değişiklik"] = change_display[
            "Değişiklik"
        ].map(label_change_type)
