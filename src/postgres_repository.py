...
def get_standard_products() -> pd.DataFrame:
    ...
    return sanitize_product_rate_frame(_query_df(
        """
        SELECT
            p.id,
            b.name AS bank_name,
...
# FINANCE_SCENARIO_REPOSITORY_V1
def get_latest_finance_scenarios(
    product_ids: list[int] | None = None,
) -> pd.DataFrame:
    """
    Finansman urunleri icin en son dogrulanmis
    ornek/canli hesaplama snapshotlarini dondurur.

    Canonical standard_products verisini degistirmez.
    """
    columns = [
        "id",
        "product_id",
    ...
    frame = _query_df(
        ...
        SELECT
            ...
            source_note,
            checked_at
        FROM bansa.v_latest_finance_scenarios
        {where_sql}
        ORDER BY
            product_id,
            scenario_key,
            input_variant NULLS FIRST,
            checked_at DESC
        """
    )
...
def _rule_query(
    ...
    """
    Kural tablosunu ürün bilgileriyle birlikte okur.

    Finance Data Accuracy V2 ile bazı kural tablolarının (özellikle
    product_pricing_tiers ve finance_fact_evidence) kendi ``source_url``
    alanı vardır. Ürün sayfasını da aynı ``source_url`` adıyla SELECT etmek
    pandas'ta aynı isimli iki sütun üretir; bu durumda ``frame['source_url']``
    Series yerine DataFrame döner. Ürün kaynağını bu nedenle ayrı
    ``product_source_url`` adıyla tutuyor, kural kaynağı boşsa sadece fallback
    olarak kullanıyoruz.
    """
    ...
    if "product_source_url" in frame.columns:
        if "source_url" not in frame.columns:
            frame["source_url"] = frame["product_source_url"]
        else:
            source_text = frame["source_url"].fillna("").astype(str).str.strip()
            missing = source_text.eq("")
            if missing.any():
                frame.loc[missing, "source_url"] = frame.loc[
                    missing, "product_source_url"
                ]
...
def get_standard_product_rule_sets(
    ...
    try:
        evidence = _rule_query(
            "finance_fact_evidence",
            "b.name, p.product_name, r.fact_key",
            product_ids,
        )
    except Exception:
