...
def _document_snapshot(driver: Any) -> dict[str, Any]:
    ...
    try:
        ...
        if isinstance(value, str):
            # Basit WebDriver sarmalayıcıları/test doubles yalnızca
            # document.readyState döndürebilir. DOM'u page_source'tan
            # alırken bu durum da kullanılabilir sayılmalıdır.
            value = {"ready_state": value}
    ...
    if not body_text and html:
        # JS snapshot alınamayan kısa yönlendirme anlarında page_source
        # yine kullanılabilir olabilir; yalnız stabilite kontrolü için
        # yaklaşık görünür metni türetiriz.
