...
def is_detail_candidate(
    ...
    # Kampanya detay URL'sinin path kısmında boşluk bulunmamalıdır.
    # Emlak Katılım liste sayfasında zaman zaman href yerine kampanya
    # başlığının kendisinin URL path'ine yazıldığı bozuk kartlar
    # yayınlanabiliyor. Literal veya %20 ile kodlanmış boşlukları
    # burada reddederek ana sayfaya yönlenen sahte detayları engelleriz.
    decoded_path = unquote(urlparse(url).path or "")
...
def discover_from_html(
    *,
    ...
    for link in soup.find_all("a", href=True):
        ...
        # Bazı bankaların kampanya liste sayfası yalnızca güncel
        # katalog görevi görür ve kart üzerinde tarih yazmaz.
        # Config'te listing_presence_implies_active=true ise liste
        # üzerinde bulunmak aktiflik kanıtı olarak kaydedilir.
        #
        # Detay sayfasında açık tarih varsa campaign_status katmanı
        # tarih bilgisini listing status'tan önce değerlendirir.
        if (
            listing_status == "unknown"
            and bank.get("listing_presence_implies_active") is True
        ):
            listing_status = "active"
            evidence = (
                "Güncel kampanya liste sayfasında keşfedildi; "
                "liste üzerinde bulunması aktiflik kanıtı kabul edildi."
            )
