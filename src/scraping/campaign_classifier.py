...
def _tom_primary_content(title: str, body: str) -> str:
    """
    T.O.M./Hadi sayfalarında navigasyon ve "İlginizi Çekebilir"
    bölümleri başka kampanyalara ait indirim/iade kelimeleri içerir.
    Sınıflandırmada yalnızca gerçek kampanyanın ana içeriğini kullanır.
    """
...
def classify_campaign_record(
    *,
    ...
    is_tom_hadi = contains_any(
        source_text,
        TOM_SOURCE_TERMS,
    )
    if is_tom_hadi:
        body_text = _tom_primary_content(
            title_text,
            body_text,
        )
