...
def extract_campaign_dates(text: str) -> tuple[date | None, date | None, str]:
    ...
    # "1 Temmuz - 31 Temmuz 2026" biçiminde başlangıç
    # tarafında yılın tekrar edilmediği tarih aralıkları.
    ...
    if partial_named_range:
        ...
        start_month = MONTHS.get(
            search_key(partial_named_range.group(2))
        )
        end_month = MONTHS.get(
            search_key(partial_named_range.group(4))
        )
        if start_month and end_month:
            # T.O.M./Hadi gibi bazı siteler yıl aşan kampanyalarda
            # başlangıç yılını yazmayıp yalnızca bitiş yılını gösteriyor:
            # "03 Aralık - 31 Ağustos 2026"
            # Bu ifade 03.12.2025 -> 31.08.2026 anlamına gelir.
            start_year = end_year
            ...
            try:
                ...
                return start, end, partial_named_range.group(0)
            except ValueError:
                pass
