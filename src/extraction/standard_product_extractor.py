...
def extract_amounts(
    ...
    # Para tokeni baska bir sayinin ortasindan
    # baslayamaz veya sayinin ortasinda bitemez.
    #
    # Eski false-positive:
    #   400.001 TL
    # greedy backtracking sonrasinda "0.001"
    # olarak eslesebiliyordu.
    money = (
        r"(?<![\d.])"
        r"(\d{1,3}(?:\.\d{3})+|\d+)"
        r"(?:,\d{1,2})?"
    ...
    def is_reference_value_band(
        match: re.Match[str],
    ) -> bool:
        """
        Bir TL araliginin urunun min/max finansman
        limiti yerine finanse edilen varligin
        referans deger bandini ifade edip
        etmedigini kontrol eder.

        Ornek:
          Nihai fatura degeri 400.001 TL ile
          800.000 TL arasinda olan motosiklet
          finansmanlari icin azami vade 36 ay.

        Buradaki 400.001-800.000 TL urun limiti
        degil, nihai fatura degeri bandidir.
        """

        # Tum regex match'i "finansman..." gibi daha
        # onceki bir kelimeden baslayabilir.
        # Semantik baglami ilk para grubunun gercek
        # baslangicina gore incele.
        try:
        ...
        # extract_amounts() normalize_text() kullaniyor.
        # Guard da Turkce/ASCII farkindan etkilenmemeli.
        #
        # PowerShell/stdin Unicode farklarindan da
        # bagimsiz olmak icin codepoint tabanli
        # transliteration kullanilir.
        semantic_prefix = (
            prefix
            .casefold()
            .translate(
                {
                    0x0131: ord("i"),  # dotless i
                    0x015f: ord("s"),  # s-cedilla
                    0x011f: ord("g"),  # g-breve
                    0x00fc: ord("u"),  # u-diaeresis
                    0x00f6: ord("o"),  # o-diaeresis
                    0x00e7: ord("c"),  # c-cedilla
                    0x0307: None,      # combining dot
                }
            )
        )
        ...
        reference_value_patterns = (
            ...
            r"(?:arac|tasit|motosiklet)\s+"
            r"(?:degeri|bedeli)\s*$",

            r"(?:ekspertiz|kasko)\s+"
            r"degeri\s*$",

            r"(?:konut|gayrimenkul|tasinmaz)\s+"
            r"(?:degeri|bedeli)\s*$",
    ...
    for pattern in range_patterns:
        ...
        # Bir TL aral??? yaln?zca metinde "finansman"
        # kelimesi ge?ti?i i?in ?r?n min/max limiti
        # say?lamaz. Aral?k fatura/ara?/ekspertiz vb.
        # referans de?er band?ysa flat product amount
        # alanlar?na ta??nmaz.
        if is_reference_value_band(
            match
        ):
            continue
    ...
    max_patterns = (
        ...
        # Oran/vade tablo basliklarini azami urun tutari
        # olarak yorumlama.
        #
        # False-positive ornek:
        #   Finansman Orani Azami Oran (Ay)
        #   0 TL - 400.000 TL
        #
        # Gercek kullanim korunur:
        #   finansman icin azami 250.000 TL
        rf"(?:"
        rf"finansman\b(?!\s+oran[\u0131i]?\b)"
        rf"|kulland[\u0131i]r"
        rf")"
        rf"[^.!?]{{0,50}}"
        rf"(?:maksimum|azami|en\s+fazla)"
