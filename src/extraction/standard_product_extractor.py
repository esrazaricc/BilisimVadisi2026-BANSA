...
def extract_maturity(
    ...
    max_patterns = (
        ...
        r"(?:\s+vade)?\s*[:\-]?\s*(\d{1,3})\s*ay",
        r"\b(\d{1,3})\s*aya?\s+kadar\s+vade",
...
def extract_housing_finance_rules(
    ...
    for table in soup.find_all("table"):
        cells_all = table.find_all(["th", "td"])
        header_key = _search_key(
            " ".join(
                cell.get_text(" ", strip=True)
                for cell in cells_all[:4]
            )
        )
        if "konut degeri" not in header_key:
            continue
        ...
        if "ilk ev" in heading_key:
            purchase_type = "first_home"
        elif "ikinci" in heading_key or "sonras" in heading_key:
            purchase_type = "additional_home"
        else:
            continue
        ...
        for row in table.find_all("tr"):
            ...
            for raw in cells[1:4]:
                ...
                if not match:
                    rates = []
                    break
                rate = parse_tr_number(match.group(1))
                if rate is None:
                    rates = []
                    break
                rates.append(rate)
            ...
            rules.append(
                {
                    ...
                    "value_band_source": cells[0],
                    "value_band_short": _short_housing_band(cells[0]),
                    "rate_ab": rates[0],
                    "rate_c": rates[1],
                    "rate_other": rates[2],
                }
    ...
    def compact(kind: str) -> str | None:
        rows = [r for r in rules if r["purchase_type"] == kind]
        if not rows:
            return None
        return " · ".join(
            f"{r['value_band_short']} "
            f"{_format_rate(r['rate_ab'])}/"
            f"{_format_rate(r['rate_c'])}/"
            f"{_format_rate(r['rate_other'])}"
            for r in rows
        )
...
def extract_standard_product(
    ...
    ) = extract_housing_finance_rules(html)
