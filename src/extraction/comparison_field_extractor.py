...
def extract_audiences(
    ...
    *,
    ...
    # AUDIENCE_RULES içinde new_customer genel taramadan bilinçli olarak
    # çıkarılmıştır; sayfa footer'ındaki "Müşteri Ol" bağlantısı yanlış
    # pozitif üretmesin. Buna karşılık başlıkta açık "yeni müşteri" veya
    # mobil üzerinden müşterimiz olma ifadesi güçlü kanıttır.
    strong_new_customer = (
        "yeni musteri" in title_key
        or "yeni musteri" in folded
        or (
            "mobil" in folded
            and any(
                phrase in folded
    ...
    mixed_existing_new_scope = any(
        phrase in folded
        for phrase in (
            "mevcut ve yeni musteri",
            "mevcut hem yeni",
            "hem mevcut hem yeni",
        )
    )
    if strong_new_customer and not mixed_existing_new_scope:
        add("new_customer", "Yeni Müşteriler")
