...
TOM_STRONG_CREDIT_TERMS = (
    "hadi taksitli alışveriş kredisi",
    "hadi taksitli alisveris kredisi",
...
def _tom_primary_content(title: str, body: str) -> str:
    """T.O.M./Hadi sayfasındaki ilgili/önerilen kampanya footer'ını keser."""
    cleaned = normalize_text(body)
    title_core = normalize_text(title).split("|", 1)[0].strip()
    if title_core:
        match = re.search(re.escape(title_core), cleaned, flags=re.IGNORECASE)
        if match:
...
def _tom_override_classification(
    title: str,
    body: str,
    source: str,
) -> ClassificationResult | None:
    """T.O.M./Hadi'nin kredi/veresiye terminolojisini güvenli öncelikle ayırır."""
    if not contains_any(source, TOM_SOURCE_TERMS):
        return None

    primary = _tom_primary_content(title, body)
    title_key = search_key(title)
    body_key = search_key(primary)
    combined_key = search_key(f"{title} {primary}")

    # Kredi kartının sonradan taksitlendirilmesi finansman ürünü değildir.
    card_signal = (
        "kredi kart" in combined_key
        and not contains_any(combined_key, TOM_STRONG_CREDIT_TERMS)
    )
    if card_signal:
        return None

    # Açık Taksitli/Mağazadan kredi ifadeleri, nakit iade gibi ek faydalar
    # bulunsa da finansman kampanyasıdır.
    strong_credit = contains_any(combined_key, TOM_STRONG_CREDIT_TERMS)
    credit_context = (
        ("kredi" in combined_key or "kredilerin" in combined_key)
        and (
            "vade" in combined_key
            or "kar orani" in combined_key
            or "kar payi" in combined_key
            or "kredi kullanim" in combined_key
            or "kredi kullandirim" in combined_key
        )
    )
    if strong_credit or credit_context:
        return ClassificationResult(
            record_kind="campaign",
            campaign_category="finance_campaign",
            comparison_eligible=True,
            confidence=0.98,
            reason="T.O.M./Hadi sayfasında açık kredi/finansman kampanyası kanıtı bulundu.",
        )

    # Veresiye ürününde 'vade farksız' doğrudan finansman kullanım avantajıdır.
    veresiye_finance = (
        "veresiye" in combined_key
        and (
            "vade farksiz" in combined_key
            or "finansman avantaji" in combined_key
        )
        and not any(
            reward_term in combined_key
            for reward_term in (
                "nakit iade",
                "tl kazan",
                "odul kazan",
    ...
    if veresiye_finance:
        return ClassificationResult(
            ...
            comparison_eligible=True,
            confidence=0.97,
            reason="Veresiye için doğrudan vade/finansman avantajı bulundu.",
        )

    # Veresiye ile harca-kazan, nakit iade veya vade farkı iadesi finansman
    # kampanyası değil; parasal indirim/iade kampanyasıdır.
    if "veresiye" in combined_key and any(
        term in combined_key
        for term in (
            "nakit iade",
            "iade edilir",
            "vade farkin bizden",
        ...
        return ClassificationResult(
            ...
            comparison_eligible=True,
            confidence=0.97,
            reason="Veresiye kullanımına bağlı ödül/iade avantajı bulundu.",
        )
...
def classify_campaign_record(
    *,
    ...
    tom_override = _tom_override_classification(
        title_text, body_text, source_text
    )
    if tom_override is not None:
        return tom_override

    if contains_any(source_text, TOM_SOURCE_TERMS):
        body_text = _tom_primary_content(title_text, body_text)
