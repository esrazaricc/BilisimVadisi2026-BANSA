...
def apply_finance_data_quality_overrides(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # ------------------------------------------------------------
    # Dünya Katılım — Araç (V21 source correction)
    # ------------------------------------------------------------
    # Resmî SSS metnindeki değer bantları yalnızca vade süresinin
    # belirlenmesine esas alınır. Bu kaynak yüzdesel finansman oranı
    # yayımlamaz; %70/%50/%30/%20 benzerliği başka bankalardan
    # taşınamaz.
    if _same(bank, "Dünya Katılım") and _same(product, "Araç Finansmanı"):
        canonical = [
            (None, 400000.0, 48),
            (400000.0, 800000.0, 36),
            (800000.0, 1200000.0, 24),
            (1200000.0, 2000000.0, 12),
        ]
        rules["amount_maturity_rules"] = [
            {
                "min_amount": lo,
        ...
        out["vehicle_finance_rules_text"] = (
            "≤ 400.000 TL: 48 ay · 400.001–800.000 TL: 36 ay · "
            "800.001–1.200.000 TL: 24 ay · 1.200.001–2.000.000 TL: 12 ay "
            "(tutarlar yalnızca vade süresinin belirlenmesine esas alınır)"
        )
        ...
        display_metadata["vehicle_value_rules"] = [
            {"max_value": hi, "min_value": lo, "max_maturity_months": months, "max_financing_ratio": None}
            for lo, hi, months in canonical
        ]
        display_metadata["vehicle_ratio_published"] = False
        display_metadata["vehicle_value_rule_scope"] = "maturity_only"
        display_metadata.pop("vehicle_blocked_above", None)
