...
COLUMN_LABELS = {
    ...
    "minimum_transaction_amount": "Minimum İşlem Tutarı (TL)",
    "maximum_transaction_amount": "Maksimum İşlem Tutarı (TL)",
    "installment_cost_text": "Vade Farkı / Ek Maliyet",
...
def prepare_display(
    ...
    display = frame.copy()
    display["display_campaign_name"] = display.apply(
    ...
    display["bank_campaign"] = (
        ...
        + " — "
        + display["display_campaign_name"].fillna("")
    )
...
# Karşılaştırma ekranına yalnızca gerçekten aktif kampanyalar alınır.
# Geçmiş/expired kayıtlar DB'de korunur fakat güncel banka
# karşılaştırmasına dahil edilmez.
...
if campaigns.empty:
    st.info("Karşılaştırılacak aktif kampanya bulunmuyor.")
    st.stop()
...
subset["display_campaign_name"] = subset.apply(
    lambda row: clean_campaign_title(
        row["campaign_name"],
        row["bank_name"],
    ),
...
subset["option_label"] = (
    ...
    + " — "
    + subset["display_campaign_name"].fillna("")
)
...
for label, column, ascending, suffix in rules:
    ...
    st.write(
        f"**{label}:** "
        f"{best['bank_name']} — "
        f"{clean_campaign_title(best['campaign_name'], best['bank_name'])} "
